...
- Loads `nida_sample.json` (fake NIDA record for demo)
...
Artifacts:
- `examples/govesb-basic/out/encrypted.json` (JSON with `ephemeralKey`, `iv`, `encryptedData`)
- `examples/govesb-basic/out/decrypted.json` (plaintext JSON matching `nida_sample.json`)
